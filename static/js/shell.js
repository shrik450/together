document.querySelectorAll('.breadcrumb-item').forEach(item => {
    ...
    trigger.addEventListener('click', (e) => {
        ...
        document.querySelectorAll('.breadcrumb-trigger[aria-expanded="true"]').forEach(other => {
            if (other !== trigger) {
...
document.addEventListener('click', (e) => {
    if (!e.target.closest('.breadcrumb-item')) {
        document.querySelectorAll('.breadcrumb-trigger[aria-expanded="true"]').forEach(trigger => {
            trigger.setAttribute('aria-expanded', 'false');
        });
    }
    if (!e.target.closest('.topbar-user')) {
        document.querySelectorAll('.topbar-user-trigger[aria-expanded="true"]').forEach(trigger => {
            trigger.setAttribute('aria-expanded', 'false');
        });
    }
});
